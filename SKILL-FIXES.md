...
### إضافة (01-09-2026) — الفلترة بحقل متخزّن جوّه `extra`

فلتر **سبب الإلغاء** كشف حالة تانية من نفس التعارض: المعيار بيطلب فلتر
multi-select على عمود "سبب الإلغاء"، والسبب **مش عمود في `logs`** — هو مفتاح
جوّه الـ JSON بتاع `extra` (`extra.reasonLabel`). أي عمود في أي Log Tab مصدره
`extra` هيقع في نفس الحالة (`extra.result` مثلاً لو اتطلب فلتر عليه بكرة).

الحل المطبَّق — نفس البنّاء، شرط واحد زيادة:
...
`json_extract` شغّالة في D1 (SQLite JSON1) — **متأكَّدة بتنفيذ فعلي** على
`ecommoda-dev-logs` يوم 01-09-2026، رجّعت التلات أسباب الموجودة في السجل.

ومعاها endpoint تالت للفلتر نفسه — `get_log_reasons` — بيرجّع `DISTINCT`
القيم **من D1 مش من المصدر الأصلي (شوبيفاي)**. القاعدة اللي تتكتب مع التعديل:

> **قايمة خيارات أي فلتر سجل تتقرا من السجل، مش من مصدر القيمة.** القيمة
> ممكن تكون اتشالت من المصدر والصفوف القديمة لسه في السجل (تبقى غير قابلة
> للفلترة)، أو اتضافت للمصدر ولسه ما اتستخدمتش (تبقى خيار بيدّي صفر نتايج
> دايمًا). الاتنين بيحصلوا في السكوت.

وقاعدة تانية: **قايمة الخيارات مش متفلترة بباقي الفلاتر المختارة** — زي قايمة
الموظفين بالظبط. لو اتفلترت، الخيارات بتختفي من تحت إيد الموظف وهو بيفلتر.

**التنفيذ الحالي للمراجعة:** `Order-Cancel` → `index.js` v2.8.0،
`buildLogFilterSQL` + `logParamsFrom` + `handleGetLogReasons`.
...
"الثيم — فاتح افتراضيًا، واستثناء واحد")
**يخص:** أي حد يقرا القاعدة ويلاقي أداة تانية حمرا فيفتكرها مخالفة.

### النص الحالي

> **استثناء وحيد مقصود: `Bosta Warehouse Return` بثيم أحمر** … الاستثناء ده
> **تجاري مش جمالي** — متعمّمهوش على أي أداة تانية، وأي طلب "غيّر لون الأداة"
> يترد عليه بالقاعدة دي.

### اللي حصل

`Order-Cancel` بقت **حمرا كمان** بقرار صريح من أحمد (01-09-2026)، بنفس شكل
المبرّر التشغيلي بالظبط: دي الأداة الوحيدة في الستاك اللي بتعمل **فعل لا رجعة
فيه** على أوردر حقيقي، والموظف بيقفز بينها وبين أدوات شكلها متطابق. اللون
تنبيه بصري قبل الفعل، مش تفضيل جمالي.

**طريقة التطبيق كانت المنصوص عليها حرفيًا** — override للـ `--accent` /
`--accent-light` / `--accent-dark` بس، وباقي المتغيّرات زي ما هي (`--red`
و`--red-light` ما اتلمسوش، فبانرات الفشل لسه بلونها المعياري).

### التعديل المقترح

النص يتحوّل من **"استثناء وحيد"** لـ **قائمة استثناءات + معيار للقبول**:

```diff
- **الاستثناء الوحيد المعتمد: `Bosta Warehouse Return` بثيم أحمر.**
+ **الاستثناءات المعتمدة (اتنين — والقائمة مقفولة إلا بقرار صريح من أحمد):**
+ | الأداة | اللون | المبرّر التشغيلي |
+ |---|---|---|
+ | `Bosta Warehouse Return` | أحمر | تمييزها عن `Bosta Warehouse Tracker` — الخلط بيسجّل شحنة راجعة غلط |
+ | `Order-Cancel` | أحمر | الأداة الوحيدة بفعل لا رجعة فيه على أوردر حقيقي — تنبيه بصري قبل الفعل |
```

ومعاها **معيار القبول** بدل المنع المطلق، عشان الطلب الجاي يتحسم بقاعدة مش
باجتهاد:

> استثناء الثيم يتقبل بشرطين مع بعض: (١) مبرّر **تشغيلي** — خلط الأداة بأداة
> تانية أو تنفيذ فعل لا رجعة فيه، مش تفضيل جمالي؛ (٢) التطبيق **override
> للـ accent بس**. أي طلب مالوش الاتنين يترد عليه بالقاعدة الافتراضية.

**ليه ⚪ تحريري:** مفيش أداة بتعمل حاجة غلط لو النص فضل زي ما هو — بس أول
مراجعة لـ `Order-Cancel` هتقراها كمخالفة للمعيار وهي قرار موثّق.
...
| ٣ · `json_extract` | استعلام فعلي على `ecommoda-dev-logs` — رجّع التلات أسباب الموجودة في السجل |
| ٧ · الثيم | قرار أحمد 01-09-2026 + كود `Order-Cancel` (`:root` — override للـ accent بس) |

آخر تحديث: 01-09-2026 — 23:40
