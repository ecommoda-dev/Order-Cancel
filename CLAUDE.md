...
**الإصدار:** Worker `v2.1.1` · الواجهة `v1.0.1`   ← الاتنين مستقلين، طبيعي يختلفوا
...
- ⚠️ **زرار "⚠️ نسخة الـ Worker مختلفة" هيفضل ظاهر على طول.** `checkWorkerVersion()`
  بتقارن `TOOL_VERSION` بتاع الواجهة (`v1.0.1`) برقم الـ Worker (`2.1.1`) وبتطلب
  تطابق حرفي — بس سكيل النقل §12 بتقول إن اختلاف الرقمين **وضع طبيعي**. بند
  مفتوح تحت.
- ✅ **`REASON_ENUM_MAP` مُقرّ من أحمد 01-09-2026** — مش تخمين. الأسماء الـ14
  اتقارنت بالقائمة الحية على شوبيفاي وطابقت حرفيًا. التوزيع النهائي: 9
  `CUSTOMER` · 2 `OTHER` · 2 `INVENTORY` · 1 `FRAUD`. **`STAFF` و`DECLINED`
  مش مستخدمين خالص** — `DECLINED` معناها في شوبيفاي "الدفع اترفض" وده
  مستحيل هنا لأن الأداة بتشتغل على `PENDING` بس.
- 🔴 **أي سبب جديد يتضاف على شوبيفاي مش هيتضاف هنا تلقائيًا.** القائمة بتتقرا
  لايف فبيظهر للموظف فورًا، والنص العربي بيتكتب صح في الميتافيلد — لكن
  `REASON_ENUM_MAP[reasonLabel]` بيرجّع `undefined` فبيقع على
  `DEFAULT_SHOPIFY_REASON = "OTHER"`. **من غير أي خطأ ولا تحذير.** فأي تعديل
  على choices الميتافيلد لازم يتبعه تعديل هنا.
...
آخر مطابقة: 01-09-2026 · `index.js` v2.1.1 · `index.html` v1.0.1
🔴 معلّقة: — لا شيء
...
2. ~~مراجعة `REASON_ENUM_MAP`~~ — ✅ **اتقفل 01-09-2026** بمراجعة أحمد.
3. **حارس للأسباب غير المصنّفة** — مقترح: `?action=diag` يقارن choices
   الحية بمفاتيح `REASON_ENUM_MAP` ويطلّع أي سبب مش مغطّى، بدل ما يفضل
   صامت. (مش متعمل — قرار مؤجَّل.)
4. **قرار في زرار اختلاف النسخة** — يا إما يتشال، يا إما يتحوّل لفحص "الـ Worker
   أقدم من الحد الأدنى اللي الواجهة محتاجاه" بدل التطابق الحرفي.
5. **تضييق Build watch paths** على `index.js` + `wrangler.toml` (سكيل النقل
   §13-ب) — مش متعمل، والافتراضي `*` معناه إن أي تعديل واجهة بينشر الـ Worker
   من غير داعي.
6. **خط الأساس** — يتكتب بعد أول تشغيل ناجح.

آخر تحديث: 01-09-2026 — 10:40
